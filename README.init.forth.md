...
There aren't a lot of places to put words, and the runtime needs to use its internal word storage for its machinery, so we just put it into the dictionary and then continue with the rest of the definition. (The alternative would have been to read it into `wbuf` and then copy it into the dictionary; two birds with one stone.) `find` pops the address of the word off the stack and looks it up, setting the success value down after it. We trust that it succeeds (`drop`), and then do `@ ,`, which gets the address of the actual assembly code for `docol`, (called the CFA) and writes that into the dictionary at `HERE`. Finally, we said that we reserve a 0 in the format, which is accomplished by `0 ,`.
